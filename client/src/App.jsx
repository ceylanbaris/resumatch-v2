...
const App = () => {
  ...
  }, [interviewMessages, interviewOpen]);
  ...
  const sanitizeData = (data) => {
      ...
      if (newData.analysis) {
          newData.analysis.matches = Array.isArray(newData.analysis.matches) ? newData.analysis.matches.map(clean) : [];
          ...
          newData.analysis = { original_score: 50, optimized_score: 50, matches: [], gaps: [], additions: [] };
      }
  ...
  const handleStartInterview = async () => {
    ...
    const systemPrompt = `Sen profesyonel, biraz zorlayıcı ama yapıcı bir teknik işe alım uzmanısın (Interviewer).
    Elimde bir aday CV'si ve bir İş İlanı var. Amacın bu adayı iş ilanı için mülakata almak.
    
    GÖREV:
    1. Adaya "Merhaba" de ve doğrudan işle ilgili ilk teknik veya davranışsal soruyu sor.
    2. Sorular adayın CV'sindeki projelere veya iş ilanındaki gerekliliklere özel olsun.
    3. Tek seferde SADECE BİR soru sor.
    4. Cevap JSON formatında değil, direkt konuşma diliyle olsun. Samimi ve profesyonel ol.`;
    ...
    try {
        ...
        setInterviewMessages([{ sender: 'ai', text: text || "Merhaba, hazırsanız başlayalım. Kendinizden kısaca bahseder misiniz?" }]);
    } catch (err) {
  ...
  const handleSendInterviewMessage = async () => {
    ...
    const systemPrompt = `Sen profesyonel bir işe alım uzmanısın. Adayla mülakat yapıyorsun.
    ...
    GÖREV:
    1. Adayın cevabını kısaca değerlendir (Örn: "Güzel bir yaklaşım, ancak şuna da değinebilirdin...").
    2. Ardından, bir önceki konuyu takip eden veya yeni bir alana geçen yeni bir soru sor.
    3. Tek seferde SADECE BİR soru sor.
    4. Kısa, net ve sohbet havasında kal.`;

    try {
        const result = await callGeminiApi({
            contents: [{ parts: [{ text: "Analiz et ve cevap ver." }] }], 
            systemInstruction: { parts: [{ text: systemPrompt }] },
        });
        
        const text = result.candidates?.[0]?.content?.parts?.[0]?.text;
        setInterviewMessages(prev => [...prev, { sender: 'ai', text: text }]);
    } catch (err) {
        setInterviewMessages(prev => [...prev, { sender: 'ai', text: "Üzgünüm, bir hata oluştu." }]);
    } finally {
        setInterviewLoading(false);
    }
  };
                ...
                <div className="p-3 sm:p-4 bg-slate-900 border-t border-slate-800">
                    <div className="relative flex items-center gap-2">
                        ...
                        <button 
                            ...
                            disabled={!userInterviewInput.trim() || interviewLoading}
                            className="absolute right-2 p-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg transition-all disabled:opacity-50 disabled:bg-transparent disabled:text-slate-600"
                        >
