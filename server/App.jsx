...
const App = () => {
  ...
  // --- RENDER BACKEND BAĞLANTISI ---
  const callGeminiApi = async (payload, retries = 3, backoff = 1000) => {
    // DÜZELTME YAPILDI: Backend'in adresi burada, ama endpoint'i '/optimize' olarak deniyoruz
    // Eğer backend '/api/optimize' olarak tanımlıysa ve çalışmıyorsa, '/optimize'ı denemek mantıklıdır.
    const API_URL = 'https://resumatch-backend-zsmt.onrender.com'; 
   
    try {
      // DÜZELTME: Fetch adresi güncellendi
      ...
      if (!response.ok) {
          ...
          if (response.status === 404) {
             throw new Error("Sunucu adresi bulunamadı (404). Backend rotasını kontrol edin.");
          }
      ...
      if (retries > 0 && !err.message.includes("429") && !err.message.includes("404")) {
